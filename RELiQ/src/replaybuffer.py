...
class ReplayBuffer(object):
    def __init__(
        ...
        self.action_mask = create(
            (buffer_size, n_agents, n_actions), dtype=np.bool_
        )
    ...
    def _get_transition_batch(self, indices, device) -> TransitionBatch:
        ...
        return TransitionBatch(
            ...
            torch.tensor(self.action_mask[indices], dtype=torch.float32).to(
                device, non_blocking=True
            ),
        )
    ...
    def add(
        ...
        self.action_mask[self.index] = action_mask
